...
class ResNet50_0(tf.keras.Model):
    ...
    def build_call(self, input_shape, training=True, intermediates_dict=None):
        ...
        output_s = tf.image.resize(output_s, inputs.shape[1:-1], align_corners=True)
        masked_inputs = inputs*output_s
        if self.semantic_mode == 'residual':
            x = inputs + masked_inputs
        elif self.semantic_mode == 'mask':
            x = masked_inputs
        else:
            raise ValueError('Unrecognized semantic mode: {}'.format(self.semantic_mode))
