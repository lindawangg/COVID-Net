...
def log_tensorboard_images(sess, test_image_summary_pos, semantic_image_tensor, log_positive,
                           test_image_summary_neg, log_negative, training_ph='keras_learning_phase:0'):
    """Runs summary ops for semantic map overlays"""
    # Log images and semantic output
    summary_pos = sess.run(
        test_image_summary_pos, feed_dict={semantic_image_tensor: log_positive, training_ph: 0})
    if len(log_negative.shape) != 1:
        summary_neg = sess.run(
            test_image_summary_neg, feed_dict={semantic_image_tensor: log_negative, training_ph: 0})
    else:
        summary_neg = summary_pos
        print('There are no negative images! Negative images are a repeat of the positive ones!')
